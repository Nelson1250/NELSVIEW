...
@app.route('/launch_3d', methods=['POST'])
def launch_3d():
    data = request.json
    if 'upload_id' not in data:
        return jsonify({'error': 'No upload ID provided'}), 400
    ...
    try:
        # Launch holofix.py in interactive mode as a separate process
        # Note: We don't provide the --output argument so it runs in interactive mode
        cmd = ['python', 'holofix.py', '--folder', upload_folder]
        ...
        return jsonify({
            'success': True,
            'message': 'Launched 3D viewer application'
        })
